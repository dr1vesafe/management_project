...
class UserManager(BaseUserManager[User, int]):
    ...
    async def update(
        ...
        if hasattr(
            user_update,
            'password'
        ) and user_update.password is not None:
            update_dict["hashed_password"] = (
                self.password_helper
                .hash(user_update.password)
            )

        if hasattr(
            user_update,
            'first_name'
        ) and user_update.first_name is not None:
            update_dict["first_name"] = user_update.first_name

        if hasattr(
            user_update,
            'last_name'
        ) and user_update.last_name is not None:
        ...
        if not safe:
            if hasattr(
                user_update,
                'is_superuser'
            ) and user_update.is_superuser is not None:
